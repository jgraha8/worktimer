...
void print_version() { printf("worktimer version %s\n", VERSION); }

void print_usage() { printf("Usage: worktimer [-v] [-f logfile.txt], type h for command help.\n"); }

void print_commands() { printf("Commands: s[tatus], l[og] <message>, r[eset], q[uit]\n"); }

void print_prompt() { printf("command> "); }

void print_log_header()
{
        fprintf(logfile, "         Start Time         Work Time (hr)            Log Message      \n");
        fprintf(logfile, "========================== ================ ===========================\n");
}
...
void session_status(int print_file)
{
        ...
        printf("Elapsed Time (hr): %.2f", hours);
        if (print_file) {
                printf("\n");
                fprintf(logfile, " %s         %.2f      ", start_time, hours);
        }
}
